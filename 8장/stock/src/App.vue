<template>
  <article>
    <nav>
      <ul>
        <li @click="setName('LG에너지솔루션')" :class="{'nav-active' : name === 'LG에너지솔루션'}">LG에너지솔루션 </li>
        <li @click="setName('KBSTAR 미국나스닥100')" :class="{'nav-active' : name === 'KBSTAR 미국나스닥100'}">KBSTAR 미국나스닥100 </li>
        <li @click="setName('케이티비네트워크')" :class="{'nav-active' : name === '케이티비네트워크'}">케이티비네트워크</li>
        <li @click="setName('하이브')" :class="{'nav-active' : name === '하이브'}">하이브</li>
      </ul>
    </nav>
...
<script>
  ...
  export default {
    ...
    components: {},
    setup() {
      let name = ref("LG에너지솔루션")
      let message = ref("")
      let targetCur = ref(0)
      let herestk = ref({})
      let herestksise = ref([])
      let loading = ref(true)
      const draw = (target=0, now) => {
